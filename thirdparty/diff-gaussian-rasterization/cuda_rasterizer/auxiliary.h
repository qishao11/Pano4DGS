	...
	float3& p_view)
{
	...
	// Near-plane cull.  0.2 assumed a metric-scale scene: upstream 3DGS is run on
	// room reconstructions several units across, where 0.2 sits harmlessly in front
	// of the camera.  Our panoramic reconstructions come out ~0.2 units across in
	// total (the depth prior sets the scale), so entire walls landed inside this
	// threshold and were culled -- three of six cubemap faces rendered black with
	// >99% of their in-frustum Gaussians dropped, while the faces whose wall
	// happened to sit at 0.25 rendered fine (panoramic_4dgs_status.md section 3.47).
	// A cull is still needed -- at and behind the optical centre the projection
	// diverges -- but it belongs far enough below any usable scene scale that it
	// stops depending on which units the reconstruction came out in.
