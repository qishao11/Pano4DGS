...
def get_loss_mapping_rgbd(config, image, depth, viewpoint):
    ...
    # NOTE: previously `1./depth * depth_pixel_mask` -- for any pixel with zero rendered depth
    # (no Gaussian coverage there, e.g. common on sparsely-covered secondary rig/cubemap views)
    # this evaluates 1./0 = inf *before* masking, and inf * 0 = NaN (not 0) in IEEE float, which
    # poisons the whole .mean(). Clamp to a safe placeholder for masked-out pixels so 1/x is
    # never evaluated at x=0; masked-in pixels are numerically identical to before.
    safe_depth = torch.where(depth_pixel_mask, depth, torch.ones_like(depth))
    safe_gt_depth = torch.where(depth_pixel_mask, gt_depth, torch.ones_like(gt_depth))
    l1_depth = torch.abs((1./safe_depth - 1./safe_gt_depth) * depth_pixel_mask).mean()
